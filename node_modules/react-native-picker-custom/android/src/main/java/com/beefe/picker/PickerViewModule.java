...
public class PickerViewModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
    ...
    private static final String IS_SHOW_LONG_TIME_TEXT = "isShowLongTime";
    private static final String PICKER_LONG_TINE_TEXT = "pickerLongTimeText";
    private static final String PICKER_LONG_TIME_TEXT_SIZE = "pickerLongTimeFontSize";
    private static final String PICKER_LONG_TIME_TEXT_COLOR = "pickerLongTimeFontColor";
    private static final String PICKER_LONG_TIME_BG = "pickerLongTimeBg";
    ...
    public void _init(ReadableMap options) {
        ...
        if (activity != null && options.hasKey(PICKER_DATA)) {
            ...
            RelativeLayout outsideLayout = (RelativeLayout) view.findViewById(R.id.outsideLayout);
            ...
            RelativeLayout textLayout = (RelativeLayout) view.findViewById(R.id.textLayout);
            TextView longTimeTV = (TextView) view.findViewById(R.id.longTimeTV);

            int barViewHeight;
            barViewHeight = barLayout.getLayoutParams().height;
            ...
            int textViewHeight;
            textViewHeight = textLayout.getLayoutParams().height;

            boolean isShowLongText = false;
            if (options.hasKey(IS_SHOW_LONG_TIME_TEXT)) {
                isShowLongText = options.getBoolean(IS_SHOW_LONG_TIME_TEXT);
            }
            if(isShowLongText){
            ...
            if (options.hasKey(PICKER_LONG_TINE_TEXT)) {
                longTimeText = options.getString(PICKER_LONG_TINE_TEXT);
            }
            longTimeTV.setText(!TextUtils.isEmpty(longTimeText) ? longTimeText : "");

            if (options.hasKey(PICKER_LONG_TIME_TEXT_COLOR)) {
                ReadableArray array = options.getArray(PICKER_LONG_TIME_TEXT_COLOR);
                int[] colors = getColor(array);
                longTimeTV.setTextColor(Color.argb(colors[3], colors[0], colors[1], colors[2]));
            }

            if (options.hasKey(PICKER_LONG_TIME_TEXT_SIZE)) {
               int pickerLongTimeFontSize = options.getInt(PICKER_LONG_TIME_TEXT_SIZE);
               longTimeTV.setTextSize(pickerLongTimeFontSize);
            }

            if (options.hasKey(PICKER_LONG_TIME_BG)) {
                ReadableArray array = options.getArray(PICKER_LONG_TIME_BG);
                int[] colors = getColor(array);
                textLayout.setBackgroundColor(Color.argb(colors[3], colors[0], colors[1], colors[2]));
            }

            longTimeTV.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
            ...
            int height = barViewHeight + pickerViewHeight + textViewHeight;
            if (dialog == null) {
